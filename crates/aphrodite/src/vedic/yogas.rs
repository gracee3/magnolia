...
pub fn identify_yogas(layer_positions: &LayerPositions) -> Vec<Yoga> {
    ...
    let planet_lons: HashMap<String, f64> = planets
        .iter()
    ...
    let get_lon = |planet_id: &str| -> Option<f64> { planet_lons.get(planet_id).copied() };
    ...
    if let (Some(jupiter_lon), Some(moon_lon)) = (get_lon("jupiter"), get_lon("moon")) {
            ...
            && (is_in_kendra(moon_lon, ascendant) || is_in_trikona(moon_lon, ascendant))
        {
            yogas.push(Yoga {
                ...
                description:
                    "Jupiter and Moon in kendras or trikonas - brings wisdom and prosperity"
                        .to_string(),
            });
        }
    }
    ...
    if let (Some(mercury_lon), Some(sun_lon)) = (get_lon("mercury"), get_lon("sun")) {
        if is_conjunction(mercury_lon, sun_lon, 15.0) {
            yogas.push(Yoga {
                ...
                yoga_type: "benefic".to_string(),
                description:
                    "Mercury and Sun in conjunction - brings intelligence and communication skills"
                        .to_string(),
            });
        }
    }
    ...
    if benefic_in_kendra && benefic_in_trikona {
        yogas.push(Yoga {
            ...
            yoga_type: "benefic".to_string(),
            description:
                "Benefic planets in both kendras and trikonas - brings power and authority"
                    .to_string(),
        });
    }
    ...
    if let (Some(venus_lon), Some(jupiter_lon)) = (get_lon("venus"), get_lon("jupiter")) {
        ...
        if venus_house == 2 || venus_house == 11 || jupiter_house == 2 || jupiter_house == 11 {
            yogas.push(Yoga {
                ...
                yoga_type: "benefic".to_string(),
                description:
                    "Wealth-giving planets in 2nd or 11th house - brings financial prosperity"
                        .to_string(),
            });
        }
    }
    ...
    if let Some(moon_lon) = get_lon("moon") {
        ...
        if benefics_around_moon >= 2 {
            yogas.push(Yoga {
                ...
                yoga_type: "benefic".to_string(),
                description:
                    "Two or more benefic planets around Moon - brings happiness and prosperity"
                        .to_string(),
            });
        }
    }
    ...
    if let Some(moon_lon) = get_lon("moon") {
        ...
        if malefics_around_moon >= 2 {
            yogas.push(Yoga {
                ...
                description: "Two or more malefic planets around Moon - may cause difficulties"
                    .to_string(),
            });
        }
    }
    ...
    if let (Some(sun_lon), Some(jupiter_lon)) = (get_lon("sun"), get_lon("jupiter")) {
        ...
        if sun_normalized >= 180.0 && sun_normalized <= 210.0 {
            if is_conjunction(sun_lon, jupiter_lon, 10.0) || is_in_kendra(jupiter_lon, ascendant) {
                yogas.push(Yoga {
                    name: "Neecha Bhanga Raj Yoga".to_string(),
                    yoga_type: "benefic".to_string(),
                    description:
                        "Debilitated planet with benefic - cancels debilitation and brings success"
                            .to_string(),
                });
            }
        }
        ...
        if let Some(lon) = get_lon(malefic) {
            ...
            if matches!(house, 6 | 8 | 12) {
                yogas.push(Yoga {
                    ...
                    yoga_type: "benefic".to_string(),
                    description: format!(
                        "{} in {}th house - turns adversity into success",
                        malefic
                            .chars()
                            .next()
                            .unwrap()
    ...
    if strong_planets.len() >= 3 {
        yogas.push(Yoga {
            ...
            yoga_type: "benefic".to_string(),
            description: "Multiple planets in angular houses - brings great achievements"
                .to_string(),
        });
    }
