...
impl<'a> TransitChart<'a> {
    ...
    pub fn draw_points(&self, draw: &Draw) {
        ...
        let point_radius = self.radix_radius
            + (self.radix_radius / inner_ratio + (self.settings.padding * transit_scale));
        ...
        for (glyph, planet_data) in &self.data.planets {
            ...
            let p_end = get_point_position(
                self.cx,
                self.cy,
                pointer_radius + ruler_r / 2.0,
                angle,
                self.shift,
            );
