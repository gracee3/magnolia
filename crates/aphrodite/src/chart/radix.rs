use nannou::prelude::*;
use nannou::lyon::math::{Angle, vector, point as lpoint};
...
impl<'a> RadixChart<'a> {
    pub fn new(cx: f32, cy: f32, radius: f32, data: &'a ChartData, settings: &'a ChartSettings) -> Self {
    ...
    pub fn draw_bg(&self, draw: &Draw) {
        if self.settings.stroke_only { return; }
    ...
    pub fn draw_universe(&self, draw: &Draw) {
        let universe = vec![
            Glyph::Aries, Glyph::Taurus, Glyph::Gemini, Glyph::Cancer, 
            Glyph::Leo, Glyph::Virgo, Glyph::Libra, Glyph::Scorpio, 
            Glyph::Sagittarius, Glyph::Capricorn, Glyph::Aquarius, Glyph::Pisces
        ];
    ...
    fn draw_sector(&self, draw: &Draw, r_out: f32, r_in: f32, start_deg: f32, end_deg: f32, color: Srgba) {
        // Calculate points
        ...
        builder.arc_to(
            vector(r_in, r_in),
            Angle::degrees(0.0),
            ArcFlags { large_arc: false, sweep: false },
    ...
    pub fn draw_points(&self, draw: &Draw) {
        ...
        let ruler_r = (self.radius / self.settings.inner_circle_radius_ratio) / self.settings.ruler_radius;
        let inner_ring_r = self.radius / self.settings.inner_circle_radius_ratio;
        let point_radius = self.radius - (inner_ring_r + 2.0 * ruler_r + (self.settings.padding * self.settings.symbol_scale));
        ...
        for (glyph, planet_data) in &self.data.planets {
            ...
            let p_end = get_point_position(self.cx, self.cy, ptr_r - ruler_r/2.0, angle, self.shift);
    ...
    pub fn draw_axis(&self, draw: &Draw) {
         ...
         let indices = [(0, Glyph::Ascendant), (3, Glyph::IC), (6, Glyph::Descendant), (9, Glyph::MC)];
         
         let axis_radius = self.radius + ((self.radius / self.settings.inner_circle_radius_ratio) / 4.0);
         ...
         for (i, glyph) in indices.iter() {
             ...
             let text_pos = get_point_position(self.cx, self.cy, axis_radius + (20.0 * self.settings.symbol_scale), angle, self.shift);
             draw_glyph(draw, *glyph, text_pos, self.settings.symbol_scale * 14.0, color, self.settings.stroke_axis);
         }
    }
    ...
    pub fn draw_cusps(&self, draw: &Draw) {
        ...
        let inner_r = self.radius / self.settings.indoor_circle_radius_ratio;
        let ruler_r = (self.radius / self.settings.inner_circle_radius_ratio) / self.settings.ruler_radius;
        let outer_limit = self.radius - (self.radius / self.settings.inner_circle_radius_ratio + ruler_r);
