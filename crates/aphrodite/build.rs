...
fn main() {
    ...
    out.push_str("use nannou::lyon::math::point as lpoint;\n");
    out.push_str("use nannou::lyon::path::Path;\n\n");

    out.push_str("#[derive(Clone, Copy, Debug)]\n");
    out.push_str("pub enum GlyphOp {\n");
    out.push_str("    M(f32, f32),\n");
    out.push_str("    L(f32, f32),\n");
    out.push_str("    Q(f32, f32, f32, f32),\n");
    out.push_str("    C(f32, f32, f32, f32, f32, f32),\n");
    out.push_str("    Z,\n");
    out.push_str("}\n\n");

    out.push_str("#[derive(Clone, Copy, Debug)]\n");
    out.push_str("pub struct GlyphBounds {\n");
    out.push_str("    pub min_x: f32,\n");
    out.push_str("    pub min_y: f32,\n");
    out.push_str("    pub max_x: f32,\n");
    out.push_str("    pub max_y: f32,\n");
    out.push_str("}\n\n");

    out.push_str("pub fn build_path(ops: &[GlyphOp]) -> Path {\n");
    out.push_str("    let mut builder = Path::builder();\n");
    out.push_str("    let mut open = false;\n");
    out.push_str("    for op in ops {\n");
    out.push_str("        match *op {\n");
    out.push_str("            GlyphOp::M(x, y) => {\n");
    out.push_str("                if open {\n");
    out.push_str("                    builder.end(false);\n");
    out.push_str("                }\n");
    out.push_str("                builder.begin(lpoint(x, y));\n");
    out.push_str("                open = true;\n");
    out.push_str("            }\n");
    out.push_str("            GlyphOp::L(x, y) => builder.line_to(lpoint(x, y)),\n");
    out.push_str("            GlyphOp::Q(x1, y1, x, y) => builder.quadratic_bezier_to(lpoint(x1, y1), lpoint(x, y)),\n");
    out.push_str("            GlyphOp::C(x1, y1, x2, y2, x, y) => {\n");
    out.push_str("                builder.cubic_bezier_to(lpoint(x1, y1), lpoint(x2, y2), lpoint(x, y));\n");
    out.push_str("            }\n");
    out.push_str("            GlyphOp::Z => {\n");
    out.push_str("                builder.close();\n");
    out.push_str("                builder.end(true);\n");
    ...
    for spec in specs {
        ...
        for op in &collector.ops {
            match *op {
                Op::M(x, y) => {
                    out.push_str(&format!("    GlyphOp::M({}, {}),\n", fmt_f32(x), fmt_f32(y)));
                }
                Op::L(x, y) => {
                    out.push_str(&format!("    GlyphOp::L({}, {}),\n", fmt_f32(x), fmt_f32(y)));
                }
                Op::Q(x1, y1, x, y) => {
                    out.push_str(&format!(
                        "    GlyphOp::Q({}, {}, {}, {}),\n",
