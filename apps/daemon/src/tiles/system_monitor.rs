...
impl SystemMonitorTile {
    ...
    fn refresh_metrics(&mut self) {
        ...
        push_history(
            &mut self.net_down_history,
            total_received / (1024.0 * 1024.0),
        );
        push_history(
            &mut self.net_up_history,
            total_transmitted / (1024.0 * 1024.0),
        );
    }
}
...
impl TileRenderer for SystemMonitorTile {
    ...
    fn render_monitor(&self, draw: &Draw, rect: Rect, _ctx: &RenderContext) {
        ...
        let stats = [
            ...
            format!(
                "GPU: {:.0}% / {:.0}°C",
                self.current_gpu_util, self.current_gpu_temp
            ),
            format!(
                "NET: ↓{:.1} ↑{:.1} MB/s",
    ...
    fn render_controls(&self, draw: &Draw, rect: Rect, _ctx: &RenderContext) -> bool {
        ...
        let main_graph_height = (inner_rect.h() / 2.0
            - graph_spacing * (num_main_graphs as f32 - 1.0))
            / num_main_graphs as f32;
        ...
        draw_graph(
            draw,
            pt2(inner_rect.left(), current_y),
            inner_rect.w(),
            main_graph_height,
            &self.cpu_history,
            "CPU Global (%)",
            srgba(0.0, 1.0, 0.5, 1.0),
            100.0,
        );
