...
pub fn render(draw: &Draw, rect: Rect, state: &GlobalSettingsState, anim: &ModalAnim) {
    ...
    let form_rect = Rect::from_x_y_w_h(
        content_rect.x(),
        content_rect.y(),
        form_width,
        content_rect.h(),
    ...
    form.stepper_row(draw, "Sample Rate", &state.sample_rate.to_string());

    form.stepper_row(draw, "Buffer Size", &state.audio_buffer_size.to_string());
        ...
        match state.focus.focused {
            0 => {
                ...
                match nav {
                    ...
                    UiNav::Right => {
                        if state.sample_rate < 48000 {
                            state.sample_rate = 48000;
                        } else if state.sample_rate < 96000 {
            ...
            1 => {
                ...
                match nav {
                    UiNav::Left => {
                        if state.audio_buffer_size > 256 {
                            state.audio_buffer_size /= 2;
                        }
                    }
                    UiNav::Right => {
                        if state.audio_buffer_size < 2048 {
                            state.audio_buffer_size *= 2;
                        }
                    }
                    _ => {}
