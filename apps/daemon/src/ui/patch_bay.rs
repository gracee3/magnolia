...
pub fn render(
    ...
    let modules_rect = Rect::from_x_y_w_h(
        content_rect.left() + col_w / 2.0,
        content_rect.y(),
        col_w - 10.0,
        content_rect.h(),
    );
    let ports_rect = Rect::from_x_y_w_h(
        content_rect.x(),
        content_rect.y(),
        col_w - 10.0,
        content_rect.h(),
    );
    let patches_rect = Rect::from_x_y_w_h(
        content_rect.right() - col_w / 2.0,
        content_rect.y(),
        col_w - 10.0,
        content_rect.h(),
    );
    ...
    let patch_list = List::new(&state.patches_focus, patches_rect, patches.len(), 30.0).with_title(
        if patches_focused {
            "> CONNECTIONS <"
        } else {
            "CONNECTIONS"
    ...
    if state.staged_source.is_some() {
        if let Some(start) = staged_src_pos {
            ...
            if let Some(end) = target_port_pos {
                ...
                draw.line().start(start).end(end).color(MAGENTA).weight(2.0);
            }
        }
    }
...
pub fn handle_key(key: Key, state: &mut PatchBayModalState, patch_bay: &mut PatchBay) -> bool {
