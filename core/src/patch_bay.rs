...
impl PatchBay {
    ...
    pub fn register_module(&mut self, schema: ModuleSchema) {
        if self.modules.contains_key(&schema.id) {
            log::warn!(
                "PatchBay: Module '{}' already registered, skipping",
                schema.id
            );
            return;
        }
    ...
    pub fn unregister_module(&mut self, module_id: &str) {
        ...
        self.patches
            .retain(|p| p.source_module != module_id && p.sink_module != module_id);
    }
    ...
    pub fn connect(
        ...
        let sink_schema = self
            .modules
            .get(sink_module)
            .ok_or_else(|| PatchBayError::ModuleNotFound(sink_module.to_string()))?;

        // Find ports
        let src_port = source_schema
            .ports
            .iter()
            .find(|p| p.id == source_port)
            .ok_or_else(|| {
                PatchBayError::PortNotFound(source_module.to_string(), source_port.to_string())
            })?;
        let snk_port = sink_schema
            .ports
            .iter()
...
mod tests {
    ...
    fn test_type_compatibility_exact_match() {
        ...
        assert!(PatchBay::types_compatible(
            &DataType::Audio,
            &DataType::Audio
        ));
    }
    ...
    fn test_connect_incompatible_types() {
        ...
        assert!(matches!(
            result,
            Err(PatchBayError::IncompatibleTypes { .. })
        ));
    }
    ...
    fn test_disconnect() {
        ...
        let source_schema = make_schema(
            "source",
            vec![make_port("out", DataType::Text, PortDirection::Output)],
        );
        let sink_schema = make_schema(
            "sink",
            vec![make_port("in", DataType::Text, PortDirection::Input)],
        );
